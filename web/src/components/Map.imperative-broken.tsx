 ...
 * Map.clustered.tsx
 * 
 * Clustered version of Map.tsx using Leaflet.markercluster
 * This is the COMPLETE implementation with clustering support.
 * 
 * To deploy:
 * 1. Install dependencies: npm install leaflet.markercluster @types/leaflet.markercluster
 * 2. Rename current Map.tsx to Map.legacy.tsx (backup)
 * 3. Rename this file to Map.tsx
 * 4. Test with varying church counts (10, 50, 200, 1000)
 */
...
export function Map({ churches, onChurchClick }: MapProps) {
  ...
  useEffect(() => {
    ...
    console.log('[Map] L.markerClusterGroup available?', typeof L.markerClusterGroup);
    
    if (typeof L.markerClusterGroup !== 'function') {
      console.error('[Map] ❌ L.markerClusterGroup is not a function! Clustering will fail.');
      console.log('[Map] Available L methods:', Object.keys(L));
    }
    
    const clusterGroup = L.markerClusterGroup({
      maxClusterRadius: 80,           // Cluster within 80px radius
      spiderfyOnMaxZoom: true,        // Expand overlapping markers at max zoom
      showCoverageOnHover: false,     // Don't show cluster bounds (cleaner UI)
      zoomToBoundsOnClick: true,      // Zoom into cluster on click
      disableClusteringAtZoom: 16,    // Stop clustering at street level (zoom 16+)
      chunkedLoading: true,           // Load markers in chunks for smooth UX
      chunkInterval: 200,             // 200ms between chunks
      animate: true,                  // Animate marker movements
      animateAddingMarkers: false,    // Skip animation on initial load (faster)
      
      // Custom cluster icon based on count
      iconCreateFunction: (cluster) => {
        const count = cluster.getChildCount();
        let size = 'small';
        let sizeClass = 'church-cluster-small';
        
        if (count > 50) {
          size = 'large';
          sizeClass = 'church-cluster-large';
        } else if (count > 10) {
          size = 'medium';
          sizeClass = 'church-cluster-medium';
        }
        
        return L.divIcon({
          html: `<div class="church-cluster ${sizeClass}" 
                      role="button" 
                      aria-label="${count} ${count === 1 ? 'church' : 'churches'} in this area"
                      tabindex="0">
  ...
  useEffect(() => {
    ...
    churches.forEach(church => {
      ...
      // Add to cluster group (automatically clusters)
      clusterGroup.addLayer(marker);
      
      // Keep reference for selection updates
  ...
  useEffect(() => {
    ...
    // Refresh clusters to ensure selected marker is visible
  ...
  useEffect(() => {
    ...
    // Zoom to show individual marker (past clustering threshold)
    map.flyTo([lat, lng], 17, { duration: 1, easeLinearity: 0.3 });
  }, [selectedChurch]);
