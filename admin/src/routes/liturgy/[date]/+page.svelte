      ...
      <div class="card p-6">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <!-- Saint of the day -->
            {#if liturgy.saint && liturgy.saint !== liturgy.liturgicalDayFr && liturgy.saint !== 'de la férie'}
              <div class="flex items-center gap-2 mt-3">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 text-amber-400 shrink-0"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                <span class="text-sm text-on-surface-variant capitalize">{liturgy.saint}</span>
                {#if liturgy.feast}
                  <span class="text-[11px] font-medium text-on-surface-variant/70 px-2 py-0.5 rounded-full bg-surface-container-highest">{liturgy.feast}</span>
                {/if}
              </div>
            {/if}
